...
function toggleShowInviteStudent() {
  showInviteStudent.value = !showInviteStudent.value; if (!showInviteStudent.value)
    studentsEmail.value = [];
}
function toggleBulkStudentUploadModal() {
  bulkStudentUploadModal.value = !bulkStudentUploadModal.value; if (!bulkStudentUploadModal.value)
    bulkFile.value = null;
}
...
async function handleInviteStudent() {
  ...
  if (examServerSuccess.value) {
    toggleShowInviteStudent(); await getExam({
...
function toggleSendModal() {
  sendModal.value = !sendModal.value; if (!sendModal.value)
...
watch(() => examTitle.value, async (newTitle) => {
  currentId.value = exams.value.find(e => e.examName === newTitle)._id; await getExam({
    id: currentId.value,
  }); router.push(`/monitoring-results/${currentId.value}`);
});
watch(() => currentExam.value, (e) => {
  if (e)
    accessValue.value = e.access;
});

onMounted(async () => {
  await getExams(); if (routes.params.id) { exam.value = exams.value.find(exam => exam._id === routes.params.id); examTitle.value = currentExam.value.examName; }
  else { examTitle.value = exams.value[0]?.examName || ""; }
});
</script>
...
<template>
          ...
          <main class="py-8 px-4">
              ...
              <section class="flex gap-6 mt-4">
                <div class="flex flex-col gap-3 w-full max-w-[350px]">
                  <div class="flex gap-6 items-center w-full justify-between text-black text-xl">
                    <p class="font-light">
                      Exam key
                    </p><USkeleton v-if="examServerLoading" class="w-20 h-10" /><p v-else>
                      {{ currentExam?.examKey }}
                    </p>
                  </div>
                  <div class="flex gap-6 items-center w-full justify-between text-black text-xl">
                    <p class="font-light">
                      Access
                    </p><USkeleton v-if="examServerLoading" class="w-20 h-10" /><p v-else class="capitalize">
                      {{ currentExam?.access }}
                    </p>
                  </div>
                  <div class="mt-4">
                    <h4 class="font-bold">
                      Student Status
                    </h4><div class="flex gap-2 w-full mt-3">
                      <div v-if="examServerLoading" class="w-full h-full flex items-center justify-between">
                        <USkeleton class="w-full h-[100px]" />
                      </div><div v-for="(status, index) in studentStatus" v-else :key="index" :class="`${status.borderBottom} border-b-4 p-4 flex items-center justify-center flex-col gap-3 bg-gray-100 w-full`">
                        <p class="text-xl flex items-end">
                          <span class="font-bold text-2xl">{{ status.done }}</span>/{{ status.total }}
                        </p><p class="capitalize text-xl tracking-wide">
                          {{ status.title }}
                        </p>
                      </div>
                    </div>
                ...
                <div class="w-full px-4 flex flex-col gap-4">
                  <template v-for="(button, index) in buttonList" :key="index">
                    <UDropdownMenu v-if="button?.children" :items="button.children" :ui="{ content: 'bg-white shadow cursor-pointer', item: 'cursor-pointer text-black hover:bg-black rounded' }" :disabled="examServerLoading">
                      <AppButton :label="button.label" :left-icon="button.leftIcon" :loading="examServerLoading" theme="primary" class="w-full! items-center justify-center rounded-3xl!" />
                    </UDropdownMenu>
            ...
            <template v-if="selectSelectedTab === 'results'">
              <div class="my-8 w-full px-4 flex gap-4">
                <AppButton v-for="(button, index) in buttonListTwo" :key="index" :label="button.label" :left-icon="button.leftIcon" class="w-full! items-center justify-center rounded-3xl!" :loading="examServerLoading || documentLoading" theme="primary" @click="button.clickAction" />
              </div>
              <div class="mt-4">
                <AppTab v-model="selectedResultTab" :tabs="resultsTab" theme="variant" class="py-1 tracking-wider hover:border-b-3 hover:border-zinc-800 mr-4 text-zinc-600 hover:text-zinc-800" active-class="border-b-3 border-zinc-800 text-zinc-800 cursor-pointer" />
              </div>
              <AppTable v-if="selectedResultTab === 'student'" :columns="columns" :rows="rows" :loading="examServerLoading || documentLoading">
                <template #name-cell="{ row }">
                  <p class="cursor-pointer text-blue-600 underline" @click="openTranscript(row.original.transcript)">
                    {{ row.original.name }}
                  </p>
                </template><template #action-cell="{ row }">
                  <div class="flex gap-2 items-center">
                    <AppButton left-icon="i-lucide-file-badge" theme="primary" class="border-black border-2! rounded-4xl!" title="View Transcript" :loading="documentLoading" @click="toggleTranscriptModal(row.original.transcript)" />
                  </div>
                </template>
              </AppTable>
    ...
    <template #body>
      <p class="text-center mb-3 text-gray-600">
        ...
        <div class="rounded-lg">
          <AppInput v-model="studentEmail" placeholder="Enter Students Email" @keyup.enter="addStudentEmail(studentEmail)" /><h1 class="p-2 bg-blue-500 text-white mt-4 font-bold rounded-t-lg">
            Students
          </h1><ul class="h-full max-h-[250px] p-4 flex flex-col gap-3 w-full overflow-y-scroll overflow-x-hidden">
      ...
      <AppButton label="Send" :disabled="studentsEmail.length === 0" :loading="examServerLoading" class="w-fit! px-8" @click="handleInviteStudent" />
    </template>
  </AppModal>
    ...
    <template #body>
      <ul class="flex flex-col gap-3 max-h-[300px] overflow-y-auto w-full">
        <li v-for="email in inviteEmails" :key="email" class="flex justify-between items-center border-b pb-1">
          <span>{{ email }}</span><AppButton icon="i-lucide-trash" class="text-2xl text-red-600" :loading="examServerLoading" @click="handleDropInvite(email)" />
        </li><li v-if="inviteEmails.length === 0" class="text-center italic text-gray-500">
          No invites yet
        </li>
      </ul>
    </template>
  ...
  <AppModal v-model="sendModal" title="Send exam back to student">
    <template #body>
      <p class="text-center mb-3 text-gray-600">
        This will send a link to the student which can be used to see the answers and the marking.
      </p><AppInput v-model="filterEmail" placeholder="Filter email..." /><div class="rounded-lg overflow-scroll max-h-[500px] h-auto w-full mt-2">
        <h1 class="p-2 bg-blue-500 text-white font-bold rounded-t-lg">
          Students
        </h1><p v-for="invitee in filteredInviteEmails" :key="invitee" class="flex items-center justify-between p-2 border-b-1">
  ...
  <AppModal v-model="bulkStudentUploadModal" title="Invite students to Take Exam">
    <template #body>
      <div class="flex gap-2">
        <AppInput v-model="bulkFile" type="file" accept=".csv" /><AppButton label="Download Template" class="rounded-xl! px-8" theme="secondary" @click="downloadFile('emails_and_names.csv')" />
      </div>
    </template><template #footer>
      <AppButton label="Invite" class="w-fit! px-8" :disabled="!bulkFile" theme="secondary" :loading="examServerLoading" @click="handleBulkUpload" />
    </template>
  </AppModal>
</template>
