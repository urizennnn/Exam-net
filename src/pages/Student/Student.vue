        ...
        >
          ...
          <div class="flex flex-col items-center gap-4 w-full p-3">
            ...
            >
              <p>Time Limit</p>
              <AppButton
                :leftIcon="`i-lucide-${showTimeLimit ? 'eye' : 'eye-off'}`"
...
const timeLimit = computed(
  () => (exam.value?.settings?.general.timeLimit || 0) * 60,
);
...
async function handleExamSubmit() {
  ...
  const { url: secureUrl } = await uploadPdfToCloudinary(pdfBlob);
  if (!secureUrl) {
...
onMounted(async () => {
  ...
  if (timeLimit.value > 0 && mode.value === "student") startTimer(true);
